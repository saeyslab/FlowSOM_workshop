...
3) Also plot the trees for your fsom49 object
```{r fig.width = 10, asp.ratio = 1}
...
7) Plot the cluster which contains the NKT cells on CD3 vs CD161
```{r fig.width = 10, asp.ratio = 1}
...
```{r fig.width = 15}
MFIs <- GetMFIs(fsom)[, channels_of_interest]
colnames(MFIs) <- get_markers(data$flowSet[[1]], colnames(MFIs))
pheatmap::pheatmap(MFIs)
```

```{r fig.width = 15}
meta_MFIs <- MetaclusterMFIs(fsom)[, channels_of_interest]
colnames(meta_MFIs) <- get_markers(data$flowSet[[1]], colnames(meta_MFIs))
pheatmap::pheatmap(meta_MFIs)
```
...
The table function gives an overview of the amount of times every value appears.
If you pass it two vectors, it will count the pairs that appear.
```{r}
table(c("a", "b", "b", "c", "b", "a"))
table(c("a", "b", "b", "c", "b", "a"),
      c("A", "B", "B", "A", "B", "B"))
...
We can use this to get an overview of the number of cells assigned to each of the clusters.
```{r}
table(GetClusters(fsom$FlowSOM))
```
...
We can use these numbers to plot:
```{r}
