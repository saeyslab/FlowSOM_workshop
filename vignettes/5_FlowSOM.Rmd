...
7) Plot the metacluster MFIs from the fsom49 object
```{r}
plot_metacluster_MFIs(fsom_t_b)
```

8) Plot the CD3+ CD19- cells from the fsom_t_b clustering (identified in 
the heatmap from the previous exercise) on CD3 vs CD19
```{r}
...
9) From the original FlowSOM, plot the cluster which contains the NKT cells on 
   CD3 vs CD161
```{r fig.width = 10, asp.ratio = 1}
PlotClusters2D(fsom$FlowSOM,
               get_channels(ff, "CD3"),
...
fsom_tube11 <- FlowSOMSubset(fsom$FlowSOM,
                             fsom$FlowSOM$metaData[[file]][1]:fsom$FlowSOM$metaData[[file]][2])
PlotStars(fsom_tube11)
```

Exercises:

10) Plot this figure for Tube 28
```{r}
file <- "../inst/extdata/21-10-15_Tube_028.fcs"
fsom_tube28 <- FlowSOMSubset(fsom$FlowSOM,
                            fsom$FlowSOM$metaData[[file]][1]:fsom$FlowSOM$metaData[[file]][2])
