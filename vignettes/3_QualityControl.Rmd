---
title: "Quality control of cytometry data"
...
Exercises:
1.1) Generate a "tube_11" object, containing the flowFrame that corresponds
to the cells of the tube 11:
```{r}

```

1.2) Generate a "tube_28" object, containing the flowFrame that corresponds
to the cells of the tube 28:
```{r}
...
We begin by listing all these fcs files:
...
Each flowframe in the flowSet can be accessed by filename:
...
1) How many cells are there in the second flowframe of the flowset object?
```{r}

```

2) In which channel were CD3 and CD19 measured?
```{r}
